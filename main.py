# -*- coding: utf-8 -*-
"""
DF 预设图文查询（/df 关键词）——无 LLM
模块化：dfpkg.{utils, catalog, repos, resolver}
数据：catalog/aliases.json, texts/texts.json, images/images.json, assets/*
"""
import os
...
@register("astrbot_plugin_df", "df-preset", "DF 预设图文查询（/df 关键词）——无 LLM", "0.3.0")
class DFPresetPlugin(Star):
    def __init__(self, context: Context, config: dict):
        ...
        logger.info(f"[DFPreset] loaded. fuzzy={self.fuzzy_threshold} canon={len(canon_terms)} alias={len(self.catalog.aliases)}")

    # ------- helpers -------
    def _build_chain(self, key: str) -> List[Any]:
        chain: List[Any] = []
        txt = self.texts.get(key)
        imgs = self.images.get_list(key)

        if txt:
            chain.append(Plain(txt))
    ...
    async def cmd_df(self, event: AstrMessageEvent):
        try:
            ...
            logger.error(f"[DFPreset] /df 失败: {e}")
    ...
    async def cmd_cat(self, event: AstrMessageEvent):
        try:
            ...
            if not items:
                await self._send(event, [Plain("目录为空")]); return
            text = "已注册别名：\n" + "\n".join(items[:50])
